...
from wisps, mail, and session state.
...
```bash
{{ cmd }} session list --json 2>/dev/null | grep -q '{{ .BindingPrefix }}deacon' && echo "EXISTS" || echo "MISSING"
```

If the deacon session does not exist, drain-ack and exit. The controller will
restart dead agents. If it exists but shows state "creating" for >2 minutes,
also drain-ack and exit — the controller will handle recovery.
...
Read the wisp timestamps. Build a picture:
- Recent burned wisp -> normal patrol loop
- Young in-progress wisp -> likely backoff wait or active work
- Very stale in-progress wisp -> likely stuck
- Idle with unread mail -> may need a nudge
- No wisps at all -> deacon may have crashed or not started
### Step 3: Decide
...
|-------------|---------|--------|
| Wisp updated recently | Healthy | Do nothing |
| Idle, young wisp | Backoff wait | Do nothing |
| Idle with unread mail | Needs nudge | Nudge |
| Stale wisp, ambiguous | Possibly stuck | Nudge |
| Very stale wisp, no recent updates | Clearly stuck | File warrant |
...
|------------|----------------|
| Check deacon existence | `{{ cmd }} session list --json | grep '{{ .BindingPrefix }}deacon'` |
