...
on your work bead. The workspace must be outside the current jj workspace root,
not nested under the polecat home checkout. Once created, **stay in your workspace.**
...
| Workspace path | `<workspace-parent>/workspaces/vg-1jp` |
...
with a focused test command before pushing.
...
```bash
# Step 0: Sync this workspace to the local default workspace commit
...
When nudged after dispatch, run `gc hook`. That lookup
