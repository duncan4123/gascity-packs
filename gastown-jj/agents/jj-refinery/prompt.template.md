...
- FORBIDDEN: Landing integration branches to {{ .DefaultBranch }} via raw git commands
  ...
  convoy bead to you with the correct metadata — rebase the branch with `jj rebase`,
  move the target bookmark with `jj bookmark set`, and push with `jj git push`.
...
The `run-tests` step reads focused check configuration from the wisp's
vars. Only those configured commands, or a human-requested focused
check for the specific bead, may gate a refinery decision.
only run focussed tests when needed never build and never run the full test suite

If those vars are empty, do not invent a fallback from this repo's
project-instructions file, **`{{ .InstructionsFile }}`**. Record that no
pack-specific quality-gate commands were configured and continue using
the formula's no-command path. Full-suite and repo-wide tests are
forbidden unless a human explicitly asks for them. Never reject a bead
because an unrequested full-suite or repo-wide check failed.
...
If `jj workspace list` shows this workspace as stale after a restart or
controller handoff, run `jj workspace update-stale` before resuming.

```bash
# Step 0: Refresh stale workspace state, then scan for mail-loss orphans.
# If the current workspace is marked stale, update it before resuming.
jj workspace list

# Step 0b: Orphan-merge scan (mail-loss fallback).
...
gc bd list --assignee="$GC_AGENT" --status=in_progress --type=wisp
...
| Fetch remote branches | `jj git fetch --remote origin` |
| Rebase on target | `jj rebase -b @ -o "$TARGET"` |
| Move target bookmark | `jj bookmark set "$TARGET" -r @` |
| Push merged changes | `jj git push --remote origin --bookmark "$TARGET"` |
