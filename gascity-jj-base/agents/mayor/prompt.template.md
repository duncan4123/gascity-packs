...
For any DoltLite-backed Gas City, beads-doltlite pack, DoltLite-backed Beads
storage, `doltlite-client`, native read fast path, DoltLite lock/maintenance, or
linked binary build work, treat the `gascity-jj-doltlite` skill and the
imported `beads-doltlite.doltlite` skill as required context. Follow those
skills before choosing commands.
