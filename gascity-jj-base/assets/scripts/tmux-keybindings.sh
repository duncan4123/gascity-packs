...
# Shows unread mail in a popup when clicking the status-right area.
...
if [ -n "$city" ]; then
    mail_cmd="$(shell_quote "$gc_cmd") --city $(shell_quote "$city") mail inbox || echo No unread mail"
else
    mail_cmd="$(shell_quote "$gc_cmd") mail inbox || echo No unread mail"
fi
mail_popup="display-popup -E -w 80 -h 20 $(shell_quote "$mail_cmd")"
