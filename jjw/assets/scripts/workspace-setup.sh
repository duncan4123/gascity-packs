...
write_workspace_runtime_files() {
    ...
    append_jjignore() {
        pattern="$1"
        grep -qxF "$pattern" "$WT/.jjignore" 2>/dev/null || printf '%s\n' "$pattern" >> "$WT/.jjignore"
    }

    append_jjignore ".jjignore"
    append_jjignore ".beads/redirect"
    append_jjignore ".beads/hooks/"
    append_jjignore ".beads/formulas/"
    append_jjignore ".logs/"
    append_jjignore "worktrees/"
    append_jjignore ".claude/"
    append_jjignore ".codex/"
    append_jjignore ".gemini/"
    append_jjignore ".opencode/"
    append_jjignore ".github/hooks/"
    append_jjignore ".github/copilot-instructions.md"
    append_jjignore "__pycache__/"
    append_jjignore "state.json"
}
...
install_rig_excludes() {
    if ! jj -R "$RIG_ROOT" root >/dev/null 2>&1; then
        return 0
    fi
    git_dir=$(jj -R "$RIG_ROOT" git root 2>/dev/null || true)
    if [ -z "$git_dir" ] || [ ! -d "$git_dir" ]; then
        return 0
    fi
    exclude_file="$git_dir/info/exclude"
    mkdir -p "$(dirname "$exclude_file")"
    touch "$exclude_file"
    if grep -q "# Gas City jjw rig infrastructure" "$exclude_file" 2>/dev/null; then
        return 0
    fi
    cat >> "$exclude_file" <<'EXCLUDE'

# Gas City jjw rig infrastructure
.jjw.yaml
.jjw/
EXCLUDE
}
